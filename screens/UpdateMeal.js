...
const UpdateMeal = (props) => {
  ...
  const [isVeg, setIsVeg] = useState(meals.vegetarian);
  const [filePath, setFilePath] = useState(meals.imageURL);
  ...
  useEffect(() => {
    props.navigation.setOptions({ title: meals.name });
  });

  const updateMeal = async (name, price, discount, time, filePath) => {
    setIsLoading(true);
    try {
      ...
      if (!filePath.includes("firebase")) {
        ...
        var old = await db.collection("meals").doc(mealId).get();
        old = old.data();
        console.log(old);
        ...
        doc["imageURL"] = loc;
        storageRef.child(old.imageURL).delete();
      }
      ...
      <ScrollView>
        <Formik
          initialValues={{
            ...
            time: meals.time.toString(),
            discount: meals.discount.toString(),
            filePath: meals.imageURL,
          }}
